...
QBCore.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    ...
    -- ADicinar chance de encontar items
    if Config.ChanceToFind then
        for k, item in pairs(Config.TrashItems) do
            if math.random(100) <= item.chance then
                Player.Functions.AddItem(k, item.amount, false)
                TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[k], 'add')
                TriggerClientEvent('QBCore:Notify', source, Lang:t(item.label))
            end
        end
    end
    ...
    --     TriggerClientEvent('QBCore:Notify', source, Lang:t("found.crypto"))
    -- end
